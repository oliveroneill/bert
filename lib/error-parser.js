...
class ErrorParser {
  ...
  parse(str) {
    ...
    str = this.removeFilePaths(str);
    str = this.cleanupScriptOutput(str);
    ...
    // After going through each parser we then cleanup output further by
    // removing variable names, control characters and extra whitespace
    rtn = this.removeUnnecessaryPhrases(rtn);
    // TODO: we have to remove unnecessary phrases in the lower case output
    // and the untouched output. It might just be easier to return the message
    // in lower case
  ...
  removeFilePaths(message) {
    let regex = new RegExp('\/*[a-zA-Z0-9_\.-]+\/[a-zA-Z0-9_\.-]*\/*',"g");
