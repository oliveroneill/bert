...
class ErrorParser {
  ...
  parse(str) {
    ...
    str = this.removeUnnecessaryPhrases(str);
    ...
    // Based on the output of the parser we then cleanup output further by
    ...
    // We remove the matched phrase out of the output line, so that we can
    // analyse the actual error message and remove variable names etc.
