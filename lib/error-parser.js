...
class ErrorParser {
   ...
   */
  parse(str) {
    // remove the username from the start of the string
    var parsedString = this.trimUsername(str);
    // keep a copy of the string before lowercase, so we can keep the parsed
    // string with its capitalisation
    var rtn = parsedString;
  ...
  trimUsername(command) {
    let startOfCommand = command.indexOf("$");
    if (startOfCommand > -1) {
      command = command.substring(startOfCommand + 2)
    }
    return command;
  }
}
